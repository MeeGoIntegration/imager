...
class KSWriter():
    ...
    def parse(self, img):
        ...
        for scr in conf['PostScripts']:
            f = open('/usr/share/img/kickstarter/custom/scripts/%s.post' %scr, 'r')
            postscript += f.read()
            postscript += "\n\n"
            f.close()
        ...
        for scr in conf['NoChrootScripts']:
            f = open('/usr/share/img/kickstarter/custom/scripts/%s.nochroot' %scr, 'r')
        ...
        ptab = ""
        if img.has_key("Part"):
            f = open("/usr/share/img/kickstarter/custom/part/%s" %img['Part'] )
            ptab = f.read()
            f.close()  
